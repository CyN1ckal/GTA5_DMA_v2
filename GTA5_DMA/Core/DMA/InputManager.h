...
public:
	...
	std::string QueryValue(Mem* mem, const char* path, e_registry_type type);
};
...
public:
	static bool InitKeyboard(Mem* mem);

	static void UpdateKeys(Mem* mem);
	static bool IsKeyDown(Mem* mem, uint32_t virtual_key_code);
};